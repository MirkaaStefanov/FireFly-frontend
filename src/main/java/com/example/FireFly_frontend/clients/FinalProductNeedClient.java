...
public interface FinalProductNeedClient {
    ...
    @PostMapping("/save")
    FinalProductNeedDTO save(@RequestParam Long finalProductId, @RequestParam Long midProductId, @RequestParam int quantity);

    @GetMapping("/all/{id}")
    List<FinalProductNeedDTO> findAllForFinalProduct(@PathVariable Long id);
}
