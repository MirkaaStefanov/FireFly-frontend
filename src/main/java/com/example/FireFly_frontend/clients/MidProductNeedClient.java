...
public interface MidProductNeedClient {
    @PostMapping("/save")
    MidProductNeedDTO save(@RequestParam Long midProductId, @RequestParam Long firstProductId, @RequestParam int quantity);

    @GetMapping("/all/{id}")
    List<MidProductNeedDTO> findAllForFinalProduct(@PathVariable Long id);
